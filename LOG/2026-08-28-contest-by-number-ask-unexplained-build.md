# [HASH] — The candidate-set ask states what "go" does: "Say go to file them all, or contest by number"

Both grep-found sites reworded — done.md's wind-down re-scan and rescan.md's candidate-set ask — with the asymmetry named in the rule: numbering explains contesting on its own, while "go" explains nothing unless the sentence says it files the whole set. The other "says go" hits in the corpus are unrelated (the run off-ramp, an INBOX check, the show-first hold) and were left alone.

Tick: done, confirmed — no candidate-set ask offers "go" without stating what it does. Exercised twice later in this same session, at the rescan and at this close's own wind-down.
Files touched: plugin/throughliner/docs/done.md, plugin/throughliner/docs/rescan.md.
Rule gate: run — a wording amendment to the candidate-set ask specimens (parents named); nothing evicted.
