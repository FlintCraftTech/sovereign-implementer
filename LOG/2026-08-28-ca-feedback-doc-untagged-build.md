...
Tick: done, confirmed — no send step in the doc lacks [PROMPT]; every section carries tags.
Files touched: plugin/throughliner/docs/feedback-and-inbox.md.
Rule gate: run — the existing tag system applied to a doc that missed it entirely (parent: the response-shape tag rules); nothing evicted.
