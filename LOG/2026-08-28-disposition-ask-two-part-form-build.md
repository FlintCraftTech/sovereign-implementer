...
Exercised in this same session, and it behaved as designed: the law-prose claims were processed under the new form and every answer was a verdict on a recommendation. One session and one user, with the rule fresh — an observation, not proof.

Tick: done, confirmed — the grep for the bare-mechanics ask finds only the rule's own statement of what it replaces.
Files touched: plugin/throughliner/docs/plan.md.
Rule gate: run — a wording amendment to the recommend step's ask rule (parent named); nothing evicted.
